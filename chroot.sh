...
#####################################
while 
    read -n1 -p  "Будем использовать далее русский язык? 
                   1 - да
                   0 - нет " lang   # sends right after the keypress
    echo ''
    [[ "$lang" =~ [^10] ]]
do
    :
...
if [[ $t_bootloader == 1 ]]; then
    ...
    if [[ $i_grub == 2 ]]; then
        ...
        clear
        lsblk -f
        read -p "Укажите диск куда установить GRUB (sda/sdX): " x_boot
...
##########################прописать хук
clear
echo " Редактируем /etc/mkinitcpio.conf , и в HOOKS добавляем encrypt и lvm2 — перед filesystems.
 Не забудьте проверить наличие хука udev или btrfs, если используете btrfs!
 Для LEGACY:
...
read -n 1 -s -r -p "Файл откроется в nano! \n Нажмите любую клавишу для открытия:"
...
if [[ $x_de == 0 ]]; then
  echo 'уcтановка DE пропущена' 
    ...
    pacman -S plasma-meta  plasma konsole --noconfirm
    clear
    ...
    pacman -S i3 i3-gaps i3status dmenu --noconfirm
    clear
    echo ""
    echo " Если желаете использовать 2 окружения тогда укажите 0  "
    echo ""
...
Фаил откроется через редактор  !vim!"
echo ""
